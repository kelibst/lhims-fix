/**
 * LHIMS Patient PDF Concurrent Extraction (PRODUCTION)
 *
 * Optimized for maximum speed while maintaining stability:
 * - Processes 3-5 patients simultaneously (configurable)
 * - Shares single login session across all concurrent operations
 * - Only calls APIs needed for PDF generation (consultations + admissions)
 * - Parallel OPD + IPD downloads within each patient
 * - Each patient gets own browser page (avoids conflicts)
 *
 * Usage:
 *   node scripts/extract-patient-pdf-concurrent.js [patient-list-file] [concurrency]
 *
 * Examples:
 *   node scripts/extract-patient-pdf-concurrent.js                    # Default: 3 concurrent
 *   node scripts/extract-patient-pdf-concurrent.js master-patient-list.txt 5  # 5 concurrent
 */
...
const CONFIG = {
  ...
  errorLogFile: path.join(__dirname, '..', 'pdf-extraction-concurrent-errors.log'),
  progressFile: path.join(__dirname, '..', 'pdf-extraction-concurrent-progress.json'),
  statsFile: path.join(__dirname, '..', 'pdf-extraction-concurrent-stats.json'),
  patientListFile: (() => {
...
async function extractSinglePatient(page, patientNo) {
  ...
  // ONLY call APIs needed for PDFs: consultations + admissions
  console.log('  [2/3] Fetching data...');
  const [consultations, admissions] = await Promise.all([
    getConsultations(page, patientId).catch(() => []),
    getAdmissions(page, patientId).catch(() => [])
  ]);
  console.log(`      → ${consultations.length} consultations, ${admissions.length} admissions`);

  // Create folder only if there's data to extract
  if (consultations.length > 0 || admissions.length > 0) {
  ...
  console.log('  [3/3] Downloading PDFs...');
  const pdfTasks = [];

  if (consultations.length > 0) {
    const opdPath = path.join(patientDir, `${patientNo}-OPD.pdf`);
    if (!fs.existsSync(opdPath)) {
      pdfTasks.push(
        downloadOPDPDF(page, patientNo, patientId, consultations, patientDir)
          .then(() => {
            result.opd_count = 1;
            return { type: 'OPD', ok: true };
          })
          .catch(err => {
            result.opd_failed = true;
            console.log(`      → OPD Error: ${err.message}`);
            return { type: 'OPD', ok: false, error: err.message };
          })
      );
    } else {
      result.opd_count = 1;
  ...
  result.success = (result.opd_count > 0 || result.ipd_count > 0);
  return result;
}
